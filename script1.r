#####
## Table 1, Figure 1 (A-H) and Figure S1 (A-D)
...
data <- read.csv("data1.csv"); dim(data)
...
df <- t(data.frame(c(nrow(data), table(data$baltimore))))
rownames(df) <- ""
colnames(df)[1] <- "Total"
df
...
mtext("100%",side=1,line=1,at=100+2,cex.lab=1,las=0,col="black")
...
mtext("50%",side=1,line=1,at=50+1,cex.lab=1,las=0,col="black")
title(paste0("(",toupper(h),")"),adj=0, cex.main=1.5)
title(i,font.main=1, cex.main=1)
title(paste0("n = ",format(length(x),big.mark=",")),adj=1,font.main=1, cex.main=1)
